...
- **Only the final turn can be unfinished** (`hasUnfinishedPayloadMessages`): ChatGPT stores a stopped, failed or abandoned reply for good as `in_progress` or `finished_partial_completion`; only `finished_successfully` means a reply ran to its end. An unfinished record earlier in the conversation is part of the record — flagging it made a real 93-message export "incomplete". The final turn is judged by its answer, so a stale tool call beside a finished answer is not streaming
- **Say where an unfinished reply ends** (`INTERRUPTED_NOTE`, `UNSAVED_REPLY_NOTE`, `interruptedWithoutReply`): a reply that saved no text becomes a stand-in turn rather than vanishing between two prompts, and a partial one ends with a note. `closeOpenFence` closes a code fence the reply left open — each reply is its own Markdown document in ChatGPT, but the export concatenates them
- **Dates are the reader's calendar day** (`formatDate`): `toISOString()` dated evening exports in the Americas tomorrow
