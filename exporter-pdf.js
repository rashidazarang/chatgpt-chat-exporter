(() => {
    ...
    function findMessages() {
        ...
        if (messages.length === 0) {
            ...
            if (conversationContainer) {
                ...
                console.log(`PDF: Fallback: found ${messages.length} potential messages in conversation container`);
            }
        }
    ...
    function extractFormattedContent() {
        ...
        if (messages.length === 0) {
            console.log('PDF: No messages found. The page structure may have changed.');
        ...
        messages.forEach((messageElement, index) => {
            ...
            if (!cleanText || cleanText.trim().length < 30) {
                console.log(`PDF: Skipping message ${index}: too short or empty`);
                return;
            }
            ...
            if (seenContent.has(contentHash)) {
                console.log(`PDF: Skipping message ${index}: duplicate content`);
                return;
            }
        ...
        for (let i = 1; i < processedMessages.length; i++) {
            ...
            if (current.sender === previous.sender) {
                ...
                console.log(`PDF: Fixed consecutive ${previous.sender} messages at positions ${i-1} and ${i}`);
            }
        }
        ...
        console.log(`PDF: Export completed: ${processedMessages.length} messages exported`);
        return html;
    }
